...
class NFC_Reader():
	...
	def read_card(self) :
		...
		self.hresult, self.response = SCardTransmit(self.hcard,self.dwActiveProtocol,COMMAND_READ_AUTH)


		self.hresult, self.response = SCardTransmit(self.hcard,self.dwActiveProtocol,COMMAND_READ)

		data = toASCIIString(self.response)

		return data[0:8]
		# print len(data)
		# print("Value: " + data +  " , Response:  " + str(self.response) + " HResult: " + str(self.hresult))
		# print "NBI : "+ ''.join(chr(i) for i in data)
...
if __name__ == '__main__':
	...
	if reader_status != 1 :
		data = simplejson.dumps({"status": "0", "message" : "Reader not detected", "npm" : "0", "read" : "0"})
	else :
		data = simplejson.dumps({"status": "1", "message" : "Reader detected", "npm" : "0", "read" : "0"})
		
		card_status = reader.get_card_status()

		if card_status != 1 :
			data = simplejson.dumps({"status": "0", "message" : "Reader detected, but card not detected", "npm" : "0", "read" : "0"})
		else :
			npm = reader.read_card()
			data = simplejson.dumps({"status": "1", "message" : "Reader and card detected", "npm" : npm, "read" : "1"})
